...
def build_map_dict_by_name(gdpinfo, plot_countries, year):
   ...
   """
    ...
    dict4= {}
    for i in plot_countries:
        dict3[plot_countries[i]] = i
    for a in gdpinfo:
        b = gdpinfo[a][year]
        if b != "":
            try:
                dict4[dict3[a]] = math.log10(float(b))
            except KeyError:
                pass
            except ValueError:
                pass
...
#编码，结束后将pass删除
    #不要忘记返回结果
def finder1(plot_countries, gdp_countries, year):
    list1 = []
    dict1 = {}
    dict2 = {}
    for i in plot_countries:
...
read1=reconcile_countries_by_name(pygal_countries,reader)
read2=finder1(pygal_countries,reader,year)
render_world_map(build_map_dict_by_name(reader,pygal_countries,year),read1,read2,year,"isp_gdp_world_name_%s.svg"%(year))
